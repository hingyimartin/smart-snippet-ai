...
export const register = async (req, res) => {
  ...
  if (!/^[a-zA-Z0-9_-]+$/.test(username) || username.length < 3 || username.length > 50) {
    return res.status(400).json({ error: 'A felhasználónév 3-50 karakter, csak betű, szám, _ és - lehet.' });
  }
  ...
  if (first_name.trim().length < 2 || last_name.trim().length < 2) {
    return res.status(400).json({ error: 'A kereszt- és vezetéknév legalább 2 karakter legyen.' });
  }

  const passwordErrors = [];
  if (password.length < 8) passwordErrors.push('Legalább 8 karakter legyen.');
  if (!/[A-Z]/.test(password)) passwordErrors.push('Legalább egy nagy betűt tartalmazzon.');
  if (!/[a-z]/.test(password)) passwordErrors.push('Legalább egy kis betűt tartalmazzon.');
  if (!/[0-9]/.test(password)) passwordErrors.push('Legalább egy számot tartalmazzon.');
  if (!/[!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?]/.test(password)) passwordErrors.push('Legalább egy speciális karaktert tartalmazzon.');

  if (passwordErrors.length > 0) {
    return res.status(400).json({ error: 'A jelszó nem elég erős.', details: passwordErrors });
  }
...
export const refresh = async (req, res) => {
  ...
  if (!token) {
    return res.status(401).json({ error: 'Refresh token hiányzik.' });
  }
